...
#include "BLEEncryptDescryptHelper.h"
...
void XOREncrypt(uint8_t *src, uint8_t *dst, uint8_t len, uint8_t *macAddress, uint8_t macLen) {
    for(uint8_t i = 0; i < len; i++){
        *(dst+i) = *(src+i)^ macAddress[i%macLen];
    }
}
...
uint8_t KeyK[16]={0x03, 0x1F, 0x18, 0x98, 0x5D, 0x2B, 0x44, 0x42, 0x86, 0x04, 0xA3, 0x55,0x76, 0x89, 0xE0, 0xA4}; // 种子
/**
 * 循环移位函数
...
void xorAndRC5Encrypt(uint8_t *src, uint8_t *dst, uint8_t len, uint8_t *macAddress, uint8_t macLen) {
    uint8_t temp[len];
    memset(temp, 0, len);
    XOREncrypt(src, temp, len, macAddress, macLen);//异或加密
    Encrypt(temp, dst, len);//RC5加密
...
void rc5AndXORDecrypt(uint8_t *src, uint8_t *dst, uint8_t len, uint8_t *macAddress, uint8_t macLen) {
    uint8_t temp[len];
    memset(temp, 0, len);
    Decrypt(src, temp, len);//RC5解密
    XORDecrypt(temp, dst, len, macAddress, macLen);//异或解密
}
