...
- (id)initWithFrame:(CGRect)frame
{
    ...
    self.swipeLeftGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeHandling:)];
    ...
    self.swipeRightGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeHandling:)];
    ...
    [self.tapGestureRecognizer requireGestureRecognizerToFail:self.swipeLeftGestureRecognizer];
    [self.tapGestureRecognizer requireGestureRecognizerToFail:self.swipeRightGestureRecognizer];
    [self addGestureRecognizer:self.tapGestureRecognizer];

    self.panGestureRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panHandling:)];
    self.panGestureRecognizer.delegate = self;
    [self.panGestureRecognizer requireGestureRecognizerToFail:self.swipeLeftGestureRecognizer];
    [self.panGestureRecognizer requireGestureRecognizerToFail:self.swipeRightGestureRecognizer];
...
- (void)drawRect:(CGRect)rect
{
    ...
    dateFormatter.locale = [[NSLocale alloc] initWithLocaleIdentifier:[[LanguageHelper shareLanguageHelper] currentLanguageName]];//根据APP当前的语言进行显示
	...
	NSString *monthTitle = [NSString stringWithFormat:@"%@ %ld", [monthTitles objectAtIndex:(month - 1)], (long)year];
    //// Month Header Drawing
    //[monthTitle sizeWithFont:monthFont]
    
    CGSize monthTitleSize = [monthTitle sizeWithAttributes:@{NSFontAttributeName:monthFont}];
...
- (void) setCurrentDate:(NSDate *)currentDate
{
    ...
    if (needsRedraw)
    {
        ...
        self.selectionView.currentDate = currentDate;
        [self setNeedsDisplay];
        [self periodUpdated];
...
- (void) periodSelectionChanged: (CGPoint) point
{
    ...
    {
        PMPeriod *newPeriod = [PMPeriod periodWithStartDate:self.period.startDate endDate:newDate];
        newPeriod.isUserSelected = YES;
        self.period = newPeriod;
    }
    else
    {
        PMPeriod *newPeriod = [PMPeriod oneDayPeriodWithDate:newDate];
        newPeriod.isUserSelected = YES;
        self.period = newPeriod;
    }
}
...
//                                                                                 subtype:PMThemeBackgroundSubtype];
    ...
    NSInteger startIndex = [[self.calendarView.allowedPeriod.startDate dateWithoutTime] daysSinceDate:monthStartDate] + weekdayOfFirst - 1;
    NSInteger endIndex   = [[self.calendarView.allowedPeriod.endDate dateWithoutTime] daysSinceDate:monthStartDate] + weekdayOfFirst - 1;
